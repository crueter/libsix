...
public class SixSwerveSim {
    public class SimSwerveModule {
        ...
        {
            SteerMotor = new DCMotorSim(DCMotor.getKrakenX60(1), steerGearing, steerInertia);
            DriveMotor = new DCMotorSim(DCMotor.getKrakenX60(1), driveGearing, driveInertia);
    ...
    public SixSwerveSim(Translation2d[] wheelLocations,
                               Pigeon2 pigeon,
                               DrivetrainConfiguration driveConstants,
                                List<SixSwerveModule> modules) {
        PigeonSim = pigeon.getSimState();
        ModuleCount = modules.size();
        m_modules = new SimSwerveModule[ModuleCount];
        for (int i = 0; i < ModuleCount; ++i) {
            m_modules[i] = new SimSwerveModule(driveConstants.SteerRatio, 0.0001,
                                            0.25, modules.get(i).Config.SteerInverted,
                                            driveConstants.DriveRatio, 0.001,
                                            0.25, modules.get(i).Config.DriveInverted);
        }
     ...
     */
    public void update(double dtSeconds, double supplyVoltage, List<SixSwerveModule> modulesToApply) {
        if (m_modules.length != ModuleCount) return;

        SwerveModuleState[] states = new SwerveModuleState[ModuleCount];
        /* Update our sim devices */
        for (int i = 0; i < ModuleCount; ++i) {
            TalonFXSimState steerMotor = ( (SixTalonFX) modulesToApply.get(i).getSteerMotor()).getSimState();
            TalonFXSimState driveMotor = ( (SixTalonFX) modulesToApply.get(i).getDriveMotor()).getSimState();
            CANcoderSimState cancoder = ( (SixCANCoder) modulesToApply.get(i).getSteerEncoder()).getSimState();
